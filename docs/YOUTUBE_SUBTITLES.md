...
- after a one-second wait without a captured body, can call the YouTube player's caption module and select a track once for that video generation; current YouTube Web subtitle requests may require a video-bound `pot`, so the bridge prefers a `getAudioTrack().captionTracks` entry whose `baseUrl` already contains `pot` and briefly waits for the signed equivalent if `getOption("captions", "track")` is still unsigned. This may prompt YouTube itself to request captions, but TranslateFlow does not author a timedtext fetch, manufacture a PO token, or nudge while subtitle mode is `off`.
