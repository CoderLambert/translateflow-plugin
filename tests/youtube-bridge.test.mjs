...
test("nudge selection strips caption URLs so the YouTube player owns the signed request", () => {
  ...
  assert.equal(parser.selectNudgeTrack(raw, [raw]), raw);
  assert.deepEqual(JSON.parse(JSON.stringify(parser.nudgeTrackSelection(raw))), { languageCode: "en", kind: "asr" });
  assert.equal("baseUrl" in parser.nudgeTrackSelection(raw), false);
});
