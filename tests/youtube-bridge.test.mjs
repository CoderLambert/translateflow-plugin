...
function loadPage({ videoId = "bridge-a", fetchImpl = null, xhrClass = null } = {}) {
  ...
  const track = {
    ...
    baseUrl: `https://www.youtube.com/api/timedtext?v=${videoId}&lang=en&pot=test-token`
  };
  const player = {
...
test("nudge track selection waits for a pot-bearing equivalent of the selected track", () => {
  const context = vm.createContext({ console, TextEncoder, URL, DOMParser: undefined });
  vm.runInContext(readFileSync(TIMEDTEXT, "utf8"), context);
  const parser = context.__TRANSLATE_FLOW_YOUTUBE_TIMEDTEXT__;
  const unsigned = { languageCode: "en", kind: "captions", vssId: ".en", baseUrl: "https://www.youtube.com/api/timedtext?v=x&lang=en" };
  const signed = { ...unsigned, baseUrl: "https://www.youtube.com/api/timedtext?v=x&lang=en&pot=fresh-token" };

  assert.equal(parser.hasPoToken(unsigned), false);
  assert.equal(parser.hasPoToken(signed), true);
  assert.equal(parser.selectNudgeTrack(unsigned, [unsigned, signed]).baseUrl, signed.baseUrl);
  assert.equal(parser.selectNudgeTrack(unsigned, [unsigned]), null);
});
